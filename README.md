...
| 任意 | 两个快捷方式 | 选择器里的「只切换模型（不交接）」：只切模式并打开 Codex，不动任何任务历史 |

- 选择器有三个出口：**「交接并切换」**（勾选任务后同步并切到目标模式）、
  **「只切换模型（不交接）」**（只想换模型时用，不读任务、不写历史、不推进游标）、
  **「仅打开 Codex（继续当前模式）」**（保持当前模式，只启动适配器并打开上次任务）。
  没勾任务就点「交接并切换」会被提示改用「只切换模型（不交接）」。
