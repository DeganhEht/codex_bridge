...
アダプターは `HTTPS_PROXY` / `HTTP_PROXY` / `NO_PROXY` を読み取ります。443 への直結が
遮断されていてローカルプロキシ（例: `http://127.0.0.1:7892`）が動いていれば経由し、
プロキシが無ければ直結のままです。ヘルスチェックに `proxyConfigured` / `proxyRequests` が
出て、プロキシ関連のエラーも上記の `adapter-compat-<タイムスタンプ>.txt` に記録されます。
