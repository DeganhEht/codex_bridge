...
| 任意 | どちらでも | 「只切换模型（不交接）」= タスク履歴に触れずにモードだけ切り替えて Codex を開く |

- 選択画面の出口は 3 つです: **「交接并切换」**（選択したタスクを同期して切り替え）、
  **「只切换模型（不交接）」**（モデルだけ切り替え。タスクを読まず、履歴もカーソルも変更しません）、
  **「仅打开 Codex（继续当前模式）」**（現在のモードのまま開く）。タスクを選ばずに交接ボタンを
  押すと、切り替えだけのボタンを案内します。
